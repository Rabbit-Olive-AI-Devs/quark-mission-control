...
### 3.3 API Route Bugs & Inconsistencies

1. **Inconsistent "failed" job counting** — `/api/snapshot` counts failed as `status !== "ok" && status !== "idle" && status !== "unknown"` but `/api/cron` counts failed as `status !== "ok" && status !== "unknown"`. This means idle jobs are counted as "failed" in `/api/cron` but not in the snapshot, creating different numbers depending on data source.

2. **3 endpoints completely broken on Vercel** — `/api/search`, `/api/model-usage`, and `/api/cron-history` have no remote/snapshot fallback. They depend on local filesystem or SQLite DB that don't exist on Vercel.

3. **Session-log non-today dates broken on Vercel** — When requesting a date other than today, the remote path falls through to the local parser which will fail since files don't exist on Vercel.

4. **MSE-6 agent always injected synthetically** — `/api/agents` has `ensureMse6()` that injects a hardcoded agent entry if MSE-6 is not found. Uses hardcoded model `"openai-codex/gpt-5.3-codex"` and description.

5. **Local agent response missing comms** — In local mode, `/api/agents` returns `{ agents, broadcast }` but not `comms`, while the remote/snapshot path returns `comms` too. This is an inconsistency between modes.

### 3.4 SQLite Database — UNUSED
...
|---|---|---|
| **ALL API routes bypass auth** | **HIGH** | `proxy.ts` line 4: `PUBLIC_PATHS = ["/login", "/api/"]` — every `/api/*` endpoint is publicly accessible without any authentication. Anyone with the URL can read all dashboard data. |
| **Cookie validation is broken** | **HIGH** | `proxy.ts` line 21: only checks `authCookie?.value` is truthy — never validates the token. Any non-empty cookie value grants full page access. The `sha256(password + Date.now())` token generated by `/api/auth` is never verified on subsequent requests. |
| SSE resource leak | Medium | `/api/events/route.ts` stores cleanup function via cast hack on controller (`_cleanup`) but the `cancel()` callback is empty — chokidar watcher and heartbeat interval are **never cleaned up** when clients disconnect |
...
| SSE resource leak | `events/route.ts` | `cancel()` callback is empty; cleanup stored on controller via cast hack is never invoked — chokidar watchers and intervals leak on disconnect |
| Inconsistent API response shapes | `agents/route.ts` | Local mode returns `{agents, broadcast}` but remote mode returns `{agents, broadcast, comms}` |
...
### P0 — Fix Immediately (Security + Data Accuracy)

1. **Fix API auth bypass** — `proxy.ts` exempts all `/api/` routes from auth. Either validate auth on API routes too, or at minimum protect the snapshot endpoint which exposes all data.

2. **Fix cookie validation** — actually verify the auth token content, not just its existence. Currently any non-empty cookie grants access.

3. **Fix SSE resource leak** — the `cancel()` callback in `/api/events` must call the cleanup function to close chokidar watchers and clear intervals.

4. **Replace hardcoded model fallback chain** in `model-fallback.tsx` with data from `/api/command-center` (which already reads `openclaw.json`).

5. **Fix Settings page** — read model chain and workspace config from actual config files instead of hardcoded arrays.

6. **Fix Intel date navigation** — pass date param to `/api/intel` and have the parser read date-specific intel files.

7. **Remove or update warmup tracker** — the hardcoded `WARMUP_START` will be obsolete. Either make it configurable or auto-detect.

8. **Fix "5 agents deployed"** — derive count from actual agent data, not hardcoded text.

9. **Fix inconsistent "failed" job counting** between `/api/snapshot` and `/api/cron` — standardize the logic.
...
### Immediate (This Week)
- [ ] Fix API auth bypass — protect `/api/` routes
- [ ] Fix cookie validation — verify token, not just existence
- [ ] Fix SSE resource leak (chokidar watcher + interval)
- [ ] Fix hardcoded model fallback chain → read from config
- [ ] Fix Settings page hardcoded data
- [ ] Fix Intel date navigation
- [ ] Fix inconsistent "failed" job counting between snapshot and cron API
