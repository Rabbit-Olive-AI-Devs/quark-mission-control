...
/** Read actual models from cached cron list + openclaw.json default model */
function getActualModels(): { defaultModel: string; agentModels: Record<string, string> } {
  try {
    ...
    // Reuse the cached cron list instead of spawning a separate execSync
    const jobs = parseCronList();
    for (const job of jobs) {
      const agentId = job.agentId || "";
      const model = job.model || "";
      if (agentId && model && model !== "default" && model !== "DEFAULT") {
